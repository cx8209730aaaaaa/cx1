...
class App extends React.Component {
	...
	componentDidMount() {
		//默认查询table 数据
		this.props.dispatch({type: 'attendance/fetch_slfPriceHistory'});
	}
	//重置
	handleFormReset = () => {
		...
		dispatch({type: 'attendance/fetch_slfPriceHistory',});
	}
	//查询
	handleSearch = (e) => {
		e.preventDefault();
		...
		form.validateFields((err, fieldsValue) => {
			...
			dispatch({
				type: 'attendance/fetch_slfPriceHistory',
				payload:{...values} 
			});
		});
	}
	...
	render() {
		...
		const rowKey = function(data) {return data.historyId;/*主键id*/};
		return(
			<PageHeaderLayout >
        		<Card bordered={false}>
